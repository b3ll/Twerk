...
- (void)removePoof:(UIImageView *)poof
{
    [poof stopAnimating];
    [poof removeFromSuperview];
}
...
- (void)animateDeletion
{
    ...
    if ([self.dataSource respondsToSelector:@selector(collectionView:itemAtIndexPathWillBeRemoved:)]) {
        [self.dataSource collectionView:self.collectionView itemAtIndexPathWillBeRemoved:previousIndexPath];
    }
    
    UIImageView *poof = [[UIImageView alloc] init];
    poof.frame = self.currentView.frame;
//    poof.center = self.currentView.center;
    NSMutableArray *animationImages = [[NSMutableArray alloc] init];
    
    for (int i = 1; i <= 5; i++)
    {
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"poof_%d.png", i]];
        [animationImages addObject:image];
    }
    
    NSTimeInterval poofDuration = 0.5;
    poof.animationImages = animationImages;
    poof.animationDuration = poofDuration;
    [self.collectionView addSubview:poof];
    
    [poof startAnimating];
    
    // dirty hack to remove it before it tries to repeat
    [self performSelector:@selector(removePoof:) withObject:poof afterDelay:poofDuration - 0.2];
...
- (void)handleLongPressGesture:(UILongPressGestureRecognizer *)gestureRecognizer {
    switch(gestureRecognizer.state) {
            ...
            [UIView
             ...
             animations:^{
                 ...
                 if (strongSelf) {
                     ...
                     strongSelf.currentView.alpha = 0.5;
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if (CGRectContainsPoint(CGRectInset(self.collectionView.bounds, 0.0, -100), self.currentView.center))
            {
